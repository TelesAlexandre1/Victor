(function (factory) {
  ...
  const events = [
    ...
    {
      title: 'Treino Rosielli',
      start: `${currentYear}-${currentMonth}-${currentDay} 19:00:00`,
      description:
    ...
    {
      title: `Teles ${currentYear} - Product Release`,
  ...
  const appCalendarInit = () => {
    ...
    const updateTitle = currentData => {
      ...
      if (currentViewType === 'timeGridWeek') {
        ...
        const startingMonth = weekStartsDate.toLocaleString('pt-BR', {
          month: 'short'
        });
        const startingDate = weekStartsDate.getDate();
        const weekEndDate = currentData.dateProfile.currentRange.end;

        const endingMonth = weekEndDate.toLocaleString('pt-BR', {
          month: 'short'
        });
